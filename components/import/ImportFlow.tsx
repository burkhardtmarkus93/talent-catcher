...
export function ImportFlow() {
  ...
  async function handleFile(file: File) {
    ...
    if (!parsed.success || !parsed.headers || !parsed.rows || !parsed.fileType) {
      setError(parsed.error ?? t("fileReadError"));
      return;
    }
  ...
  async function handleImport() {
    ...
    if (!res.success) {
      setError(res.error ?? t("importFailed"));
      return;
    }
    ...
    <div>
      ...
      {result && (
        <div className="mb-6 rounded-lg border border-pitch/30 bg-pitch/5 px-3 py-2 text-sm text-pitch">
          {result.errorRows > 0
            ? t("resultWithErrors", { imported: result.importedRows, errors: result.errorRows })
            : t("result", { imported: result.importedRows })}
        </div>
      )}
      ...
      {stage === "mapping" && headers.length > 0 && (
        <>
          ...
          <div className="mb-8 flex items-center justify-end gap-3">
            <span className="text-sm text-muted">{t("rowsDetected", { count: rows.length })}</span>
            <Button onClick={handleImport} disabled={submitting}>
              {submitting ? t("importing") : t("startImport")}
            </Button>
          </div>
        </>
      )}
